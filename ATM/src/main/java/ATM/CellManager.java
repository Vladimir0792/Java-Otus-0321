...
public class CellManager {
    ...
    public int[] tryPutToCells(BanknoteNominal[] banknotes) {
        if (cells == null) {
            throw new AtmException("Отсутствует банкнота");
        }
    ...
    public void putToCells(int[] cnt) {
        if (cnt.length != cells.size()) {
            throw new AtmException("Не удается вставить банкноту.");
        }
    ...
    public int[] tryGetFromCells(int sum) {
        ...
        if (curSum > 0) {
            throw new AtmException("Некорректная сумма");
        }
        return result;
    }
    ...
    private List<BanknoteNominal> getFromCell(BanknoteCell cell, int cnt) {
        ...
        for (int i = 0; i < cnt; i++) {
            try {
                ...
                throw new AtmException("Внутренняя ошибка" +  banknoteCost );
            }
        }
        return result;
    }
